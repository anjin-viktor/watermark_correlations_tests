  ...
  m_colorFormat(colorFormat)
{
  ...
  m_strides.resize(1);
  m_strides[0] = m_width * (m_colorFormat == VideoFrame::Color ? 3 : 1);;
...
VideoFrame::VideoFrame(const VideoFrame &obj)
{
  m_width = obj.m_width;
  m_height = obj.m_height;
  m_colorFormat = obj.m_colorFormat;
...
{
  return m_width * (m_colorFormat == VideoFrame::Color ? 3 : 1);
}
...
VideoFrame::ColorFormat VideoFrame::colorFormat() const
{
...
/*      for (int k1 = 0; k1 < blockSize; k1++)
      ...
      for (int posX = 0; posX < blockSize; posX++)
      {
        for (int posY = 0; posY < blockSize; posY++)
        {
          int val = (int)srcData[posX + posY * blockSize];
          if (val < 0)
            val = 0;
...
VideoFrame VideoFrame::iDCT(std::vector<float> dctData, std::size_t width, std::size_t height)
{
  ...
  cv::idct(src, dst);
  std::transform(dstData.begin(), dstData.end(), res.m_data[0].begin(), [](double value) {
    int val = (int)value;
    if (val > 255)
      val = 255;
    else if (val < 0)
    ...
    if (bypassByRows)
    {
      for (int i = 0; i < height; i++)
      {
        for (int j = 0; j < width; j++)
        {
          ...
          int wr = (int)(preference[i * stride + j] * alpha);
...
std::shared_ptr<VideoFrame> VideoFrame::sub(std::shared_ptr<VideoFrame> pframeExtracted)
{
  if (!pframeExtracted)
    return std::make_shared<VideoFrame>();

  if (width() != pframeExtracted->width() || height() != pframeExtracted->height())
    return std::make_shared<VideoFrame>();

  std::shared_ptr<VideoFrame> pres = std::make_shared<VideoFrame>(m_width, m_height, m_colorFormat);

  uint8_t* pdata = &m_data[0][0];
  uint8_t* pdataExtract = pframeExtracted->data(0);
  uint8_t* pdataRes = &pres->m_data[0][0];

  std::size_t pixelSize = m_colorFormat == VideoFrame::Color ? 3 : 1;

  for (std::size_t i = 0; i < height(); i++)
  {
    for (std::size_t j = 0; j < width() * pixelSize; j++)
    {
      uint8_t val = pdata[i * m_strides[0] + j];
      uint8_t extracted = pdataExtract[i * pframeExtracted->m_strides[0] + j];
      if (val >= extracted)
        pdataRes[i * pres->m_strides[0] + j] = val - extracted;
      else
        pdataRes[i * pres->m_strides[0] + j] = 256 - (extracted - val);
    }
  }
...
std::shared_ptr<VideoFrame> VideoFrame::add(std::shared_ptr<VideoFrame> pframeAdd)
{
  if (!pframeAdd)
    return std::make_shared<VideoFrame>();

  if (width() != pframeAdd->width() || height() != pframeAdd->height())
    return std::make_shared<VideoFrame>();

  std::shared_ptr<VideoFrame> pres = std::make_shared<VideoFrame>(m_width, m_height, m_colorFormat);

  uint8_t* pdata = &m_data[0][0];
  uint8_t* pdataExtract = pframeAdd->data(0);
  uint8_t* pdataRes = &pres->m_data[0][0];
  ...
  for (std::size_t i = 0; i < height(); i++)
  {
    for (std::size_t j = 0; j < width() * pixelSize; j++)
    {
      int val = pdata[i * m_strides[0] + j];
      int add = pdataExtract[i * pframeAdd->m_strides[0] + j];
      if (val + add < 256)
        pdataRes[i * pres->m_strides[0] + j] = (uint8_t)(val + add);
      else
        pdataRes[i * pres->m_strides[0] + j] = (uint8_t)(add + val - 256);
    }
  }
