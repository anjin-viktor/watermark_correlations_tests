...
def calc_correlation(watermark, watermark_name, embedding_level, input_filename, filter_name):
    ...
    resultNC = watermark.get_correlation(watermarked_name, reference_name, 1)
    ...
    return [filename, resultLC, resultNC, resultCC, resultPC]

def calc_correlation_wo_embedding(watermark, watermark_name, embedding_level, input_filename, filter_name):
    name_prefix = "correlation_tests/" + watermark_name + "/" + str(embedding_level)
...
def get_results_file_name_wo_embedding(watermark_name, embedding_level, filter_name):
    ...
    if filter_name:
        csv_file_name = "correlation_tests/" + watermark_name + "/" + str(embedding_level) + "/correlations_wo_embedding_" + watermark_name + "_" + str(embedding_level) + "_" + filter_name + ".csv"
...
def calc_correlations_wo_embedding_multithread(watermark, watermark_name, embedding_level, filter_name):
    threadPool = ThreadPoolExecutor()
    ...
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        future = threadPool.submit(calc_correlation_wo_embedding, watermark, watermark_name, embedding_level, filename, filter_name)
        futures.append(future)

    csv_file_name = get_results_file_name_wo_embedding(watermark_name, embedding_level, filter_name)
    csv_file = open(csv_file_name, 'w', newline='', encoding='utf-8')
    writer = csv.writer(csv_file)
...
def calc_correlations_wo_embedding(watermark, watermark_name, embedding_level, filter_name):
    calc_correlations_wo_embedding_multithread(watermark, watermark_name, embedding_level, filter_name)

if __name__ == "__main__":
    embedding_level = 5
#    calc_correlations(dct_watermark, "dct_watermark", embedding_level, "")
    calc_correlations(dct_watermark, "dct_watermark", embedding_level, "dctdnoiz_5")
    calc_correlations_wo_embedding(dct_watermark, "dct_watermark", embedding_level, "dctdnoiz_5")
